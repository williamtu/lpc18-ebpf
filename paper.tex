...
XDP packet receving/sending path to the userspace, by-passing the rest of the
...
the Rx and Tx ring structures in order to receive and send packets.
In addition, all the packet data buffers used in Tx/Rx rings are allocated from a specific
...
packet buffers with contents filled to the Tx ring, then issus sendmsg() system call.
...
In summary, users of XSK needs to properly program following four
rings:
\begin{itemize}
...
maintaining the unused/available elements in umem, and GET and PUT access APIs.
...
\begin{itemize}
\item FIFO-list\_head: We reversed first 8-byte of the umem buffer as a next pointer,
...
\item FILO-ptr\_ring: Similar to Linux kernel's ptr\_ring, an array of elements are
...
\item FIFO-ptr\_array: Similar to Linux kernel's skb\_array~\cite{skbarray},
...
With the above design change, we found that using the FIFO-ptr\_array in section~\ref{memorym} with
