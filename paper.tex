...
As the send in XSK is asynchronous and send syscall only returns zero when
...
at very high frequency, because from Figure~\ref{afxdptx}, we design to check
the success of send (step 4) immediately after issuing send (step 3).
...
and keeping polling to check transmission is done, we only issue send when there are 512 outstanding
